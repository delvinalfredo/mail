...
import { editPublisher } from "../../repository/DataRepository";
...
const EditPublisher = () => {
  ...
  const handleSubmit = (e) => {
    ...
    editPublisher(newData, id).then((res) => {
    ...
    setEndTime("");
    setWeekdays("");
    message.success("Publisher Edited");
    setShow(!show);
  };
  ...
  return (
    <Content style={{ padding: 28, backgroundColor: "white" }}>
      ...
      {show && <ParticipantEdit publisherID={id} />}
    </Content>
  );
};
